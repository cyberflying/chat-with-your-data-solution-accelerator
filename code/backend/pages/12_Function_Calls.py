...
def create_chat_completion(deployment_name, messages, endpoint, key, index_name):
    ...
    client = openai.AzureOpenAI(
        base_url=f"{azure_openai_endpoint}/openai/deployments/{deployment_name}/extensions/",
...
def handle_chat_prompt(prompt):
    ...
    with st.chat_message("assistant"):
        ...
        full_response = ""
        for response in create_chat_completion(azure_openai_deployment, st.session_state.messages, azure_search_endpoint, azure_search_key, azure_search_index_name):
...
def get_customers(search_criterion, search_value):
    ...
    if r.status_code == 200:
        return st.write(pd.read_json(StringIO(r.content.decode("utf-8"))))
    else:
...
def handle_chat_prompt_with_functions(prompt):
    ...
    with st.chat_message("assistant"):
        message_placeholder = st.empty()
        full_response = ""
        response = create_chat_completion_with_functions(azure_openai_deployment, st.session_state.messages)
...
def main():
    ...
    chat_option = st.radio(label="Choose the chat option you want to try:", options=["Function Calls", "Chat with Data"])
    ...
    if st.button("Speech to text"):
        speech_contents = recognize_from_microphone(speech_key, speech_region, speech_lang)
        if speech_contents:
